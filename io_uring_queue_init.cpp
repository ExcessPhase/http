...
std::size_t io_data::getSendOffset(const io_data&_r)
{	return dynamic_cast<const io_data_send&>(_r).m_sBuffer->m_iOffset;
}
std::shared_ptr<io_data_created_buffer> io_data::getSendBuffer(const io_data&_r)
{	return dynamic_cast<const io_data_send&>(_r).m_sBuffer;
}
...
{	return *m_sIoData.insert(
		std::make_shared<io_data_send>(this, std::move(_sHandler), _rFD, _rData).get()->shared_from_this()
	).first;
}
}
}
